...
namespace extension {
    ...
    osquery::TableColumns PingTablePlugin::columns() const {
        ...
        return {
                std::make_tuple("latency", osquery::DOUBLE_TYPE, osquery::ColumnOptions::DEFAULT),
    ...
    osquery::TableRows PingTablePlugin::generate(osquery::QueryContext &request) {
        ...
        if (hosts.empty()) {
            ...
            LOG(ERROR) << "host is required to ping. Specify the host in a WHERE clause";
            return results;
        }
        ...
        for (const auto &host: hosts) {
            ...
            if (pingOutput.empty()) {
                ...
                LOG(ERROR) << "ping command did not return any result";
                return results;
            }
            ...
            if (latencies.empty()) {
                ...
                LOG(ERROR) << "no latencies found in ping response";
                return results;
            }
    ...
    bool PingTablePlugin::isValidHost(const std::string &input) {
        ...
        // host has to have a valid length
        ...
        // regex  to check that it is a valid IPAddress (IPv4 or IPv6)
        std::regex ipv4Regex(
    ...
    std::string PingTablePlugin::pingHost(std::string host) {
        ...
        if (!shell.Execute(ping_output, kPingExecPath, ping_args) ||
            ...
            LOG(ERROR) << "Failed to execute ping command. Error: " << ping_output.std_error << " Exit Code: " << ping_output.exit_code;
            return "";
        }
